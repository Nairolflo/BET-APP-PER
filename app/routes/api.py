...
from app.models import ValueBet, Match, DailySummary, Team
...
def _serialize(bet):
    ...
    return {
        ...
        "match": {
            ...
            "league": m.league_name, "date": m.match_date.isoformat(), "status": m.status
        },
...
def global_stats():
    ...
    total = len(bets); won = sum(1 for b in bets if b.status=="WON")
    ...
    return jsonify({"total_bets": total, "won": won, "win_rate": round(won/total,4) if total else 0,
...
def by_market():
    ...
    rows = db.session.query(ValueBet.market, func.count(), func.sum(
        db.case((ValueBet.status=="WON",1),else_=0)), func.sum(ValueBet.profit_units)
    ).filter(ValueBet.status!="PENDING").group_by(ValueBet.market).all()
    return jsonify([{"market":r[0],"total":r[1],"won":r[2] or 0,
        "profit":round(float(r[3] or 0),3),"roi":round(float(r[3] or 0)/r[1],4) if r[1] else 0} for r in rows])

@api_bp.route("/trigger/morning", methods=["POST"])
def trigger_morning():
    from app.scheduler.tasks import fetch_and_store_fixtures, fetch_and_store_odds, run_detection, send_detections_today
    fetch_and_store_fixtures(); fetch_and_store_odds(); run_detection(); send_detections_today()
    return jsonify({"status": "ok"})

@api_bp.route("/trigger/evening", methods=["POST"])
def trigger_evening():
    from app.scheduler.tasks import update_results, validate_bets, compute_summaries, send_summary_today
